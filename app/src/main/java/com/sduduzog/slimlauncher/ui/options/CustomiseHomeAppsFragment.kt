...
import com.sduduzog.slimlauncher.adapters.CustomiseHomeAppsAdapter
...
@AndroidEntryPoint
class CustomiseHomeAppsFragment : BaseFragment(), OnShitDoneToHomeAppsListener {

    override fun getFragmentView(): ViewGroup = customise_apps_fragment

    private val viewModelHome: CustomiseHomeAppsViewModel by viewModels()

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        setFragmentResult("appsMode", bundleOf("hide" to false))
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val adapter = CustomiseHomeAppsAdapter(this)

        viewModelHome.apps.observe(viewLifecycleOwner, Observer {
            it?.let { apps ->
                adapter.setItems(apps)
                customise_apps_fragment_add.visibility = if(apps.size < 10) View.VISIBLE else View.INVISIBLE
            } ?: adapter.setItems(listOf())
        })
        customise_apps_fragment_remove_all.setOnClickListener {
            RemoveAllAppsDialog.getInstance(viewModelHome.apps.value!!, viewModelHome).show(childFragmentManager, "REMOVE_APPS")
        }
    ...
    override fun onAppsUpdated(list: List<HomeApp>) {
        viewModelHome.update(*list.toTypedArray())
    }
    ...
    override fun onAppMenuClicked(view: View, app: HomeApp) {
        showPopupMenu(view).setOnMenuItemClickListener {
            when (it.itemId) {
                R.id.ca_menu_rename -> {
                    RenameAppDialog.getInstance(app, viewModelHome).show(childFragmentManager, "SettingsListAdapter")
                }
                R.id.ca_menu_remove -> {
                    viewModelHome.remove(app)
                }
                R.id.ca_menu_reset -> {
                    viewModelHome.reset(app)
                }
            }
            true
        }
